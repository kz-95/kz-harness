...
| [queue-and-cost-findings.md](queue-and-cost-findings.md) | Work order, delete when done | Three faults from the first real run: read-only work waiting behind work that writes, cheap tasks served as though expensive (the auto effort ladder cannot return low), and a queue that says only "2nd in line" | Picking up the queue, effort or strategy-cost work |
