...
```
branch        chore/initial-setup          (no git remote; nothing pushed)
tests         381 pass, 0 fail             (cd plugins/jev-router && npm test)
...
(it reads "the list below" and resolves), and the one machine-specific path in the repo, a username
inside a `progress.html` note, is now `~/.kzh`. `git grep fghfh` is clean.
...
```
cd plugins/jev-router && npm test                      # 381 pass expected
