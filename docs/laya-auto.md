...
Nothing here has run in the app.
 ...
 * @property {string} model                       sent as the request model: config.jevModel ('jev-1.13.0') | 'english'
 * @property {{intent:number, route:number, review:number}|null} timeoutMs
 *                                                jev: config.jevTimeoutMs per attempt for every phase; laya: null, the Laya client computes deadlines (section 4.5)
...
export const JEV_THRESHOLDS = Object.freeze({ /* the Jev column of 2.6 */ })
export const LAYA_THRESHOLDS = Object.freeze({ /* the Laya column of 2.6 */ })
export function resolveProviders(config, { policy }) { /* -> { jev: Provider, laya: Provider | null, layaError: string | null } */ }
export function validateThresholds(t, id) { /* throws Error('providers: laya.thresholds.accept: low 0.9 is above medium 0.8') */ }
export function thresholdsSchema(defaults) { /* the Schemastery object index.js uses for both */ }
```
...
A bad Laya value, a type or range error as much as an ordering error, is caught there: it leaves the Jev record intact, sets `layaError`, takes Laya Auto out of the picker, turns the shadow off, and shows the message on the Laya card (section 8).
...
- `ask(phase, state, questions, signal, used, context)` changes in five small ways:
  ...
  5. `res.meta`, when the client supplies it, is copied onto the trace.
...
- `makeJev({ onTrace, runId, emit })` becomes `makeDecider(id, { onTrace, onError, runId, emit, signal })`.
  ...
  - `onTrace` logs through `usage.logDecision` (2.5) instead of `usage.logJev`; `onError` emits the live event `{ type: 'decider-error', at, error }` (3.3) and logs nothing to `usage.jsonl`.
- **One run id per run.**
...
- `route({ ..., mode, decider = 'jev' })`:
  ...
  - While a Laya-decided run is open, from `route()`'s start to its return, it holds the sidecar (`sidecar.hold(runId)` and `release(runId)`, 7.6), so neither the idle stop nor the watchdog takes Laya away between its calls.
...
- Tasks: `orchestrator.enqueue` and `tasks.enqueue` take `decider`, `tasks.js` adds it to `SAVED`, and `run()` passes `t.decider ?? 'jev'` to `route()`, which applies `layaUnavailable` when the task runs, however long after it was queued.
...
- The HTTP routes of section 8.4 are added, and `GET /jev-router/setup` gains `laya: sidecar.status()`.

### 2.5 `router.js`, `decision.js`, `jev-review`, `adapter.js`, `usage.js`, `effort.js`, `broker.js`

- **`router.js`** `runRouted`: `const P = deps.provider ?? DEFAULT_JEV` (a Jev record for old callers), `const T = P.thresholds`.
...
- **`decision.js`** `decide({ ..., decider, provider, sink })`, with `jev` kept as an alias of `decider`:
  ...
  - Every controller call passes `answeredBy: provider.teacher ? 'jev' : provider.id` and `sink` (6.3).
  ...
  - The decision object gains `decider: provider.id`.
...
- **`plugins/jev-review/index.js`**: `createReview(decider, thresholds, unavailableReason, { outcome, modelOf })` keeps its signature.
  ...
  - A `reviewAgent` or `retryAgent` listed in `uninformative` is replaced by `pickOther({}, last.agent)`, the deterministic peer pick the fallback path already uses, and `why` says so.
...
Laya runs also record `routing.model` as the Laya model label (3.3) and `routing.deciderErrors: [{ phase, reason }]` for every Laya call that did not answer.
...
| 1060-1061 | stop as `needs_human` | `T.humanRequired`; reason text `Laya read this as needing a person (confidence 84%)`. |
...
`client.js` reads the mode too: `WhatHappened` (`client.js:1255-1262`) and `HistoryRunDetail` (`client.js:3025`) print `Laya picked <agent> (confidence 41%)` instead of `Jev picked` when `routing.decider === 'laya'`.
...
- `Laya route failed after 42000 ms: timed out (20 questions on the CPU); routing rules decide those domains`, from `'decider-error'`; a Jev failure reads `Jev route failed after <ms> ms: <message>` from the same case.
...
- Every answer marked `corrected: true` carries the pill `uncalibrated (<k> options)`, with `<k>` its own option count; the pill follows the flag, not the question's name, because Laya picks the temperature bucket by option count (4.3).
  An answer marked `informative: false` carries `too flat, filled by rules`.
- A `'decider-error'` event shows as a call card `failed after <ms> ms: <message>` with no questions.
...
| Starting, stopping or restarting | The same wait, the same bound; a stop or restart in progress is waited out, then started (7.4) | The same line |
| Still starting past `startWaitMs` | Refused | `Laya Auto did not run this: Laya was still starting after <n> s. Nothing was run. Send it again once Settings → Jev setup → Laya decision model says Running.` |
...
| The sidecar dies mid-call | As a timeout, reason `Laya stopped while answering (exit code <code>); it is restarting` | The failure line |
...
The Laya client rejects with an `Error` whose `name` is `'Error'`, whose `message` is the reason as the person should read it (`timed out after 40 s`), and whose `code` classifies it (`LAYA_TIMEOUT`, `LAYA_PREDICTED_OVER`, `LAYA_EXITED`, `LAYA_HTTP_500`, `LAYA_UNAVAILABLE`), so `describeError` and `domains.js` print the message as is.
...
export function mergeLaya({ phase, questions }, parts /* [{ key, response }] */) -> { answers, usage, meta: { requests, rows, atContextLimit } }
...
| route | task | every task-group question but `continueHandoff` | `{ task: clip 520, workspace: clip 380 }` |
| route | handoff | `continueHandoff` | `{ task: clip 300, handoff: clip 600 }`, which today never sees the handoff behind a long task |
| route | resource | `strategy`, `secondOpinion` | `{ task: clip 520, task_profile }` (the profile only when `jev.js` sent one) |
...
| `secondOpinion` noul | `second_opinion` | `route().uninformative` | The code rule (risk at or above `riskForReview`). |
...
| `disposition` | outcome domain label | `assess().uninformative` | The domain still gives authority `laya` with the raw assessment; the sample is stored with `informative: false` and left out of every standing; the review action comes from the nouls, as for Jev (6.3). |
| `reviewAgent`, `retryAgent` | review and retry picks | `assess().uninformative` | `pickOther`, the deterministic peer pick (2.5). |
| every noul but `secondOpinion` | bars that sit away from 0.5, profile fields included | `uninformative`, for the record only | Kept as answered: a flat noul falls below every Laya bar (no stop, no accept, no tool, no handoff, no human-review flag, a second review), and the profile keeps it (2.3). |
...
```js
export function createLayaClient({ sidecar, settings /* config.laya */, isLocalBusy, log, now }) -> {
...
`laya-selfcheck.js` (G2) holds fixed calls in KzH's own shapes and the maximal warm-up bodies of 7.5, built with the `jev.js` builders and rendered with `laya-questions.js`.
It exports `createProbe(connection)`, which sends those bodies with a bare SDK client (`new TypeSafeClient({ apiKey, baseURL, defaultModel: 'english', retry: { maxRetries: 0 } })`) and checks the answers, so the sidecar's warm-up and the install's step 7 (G3) can run it without `laya-client.js` (G4), which itself needs a running sidecar.
...
- **Timings**: intent, route and review milliseconds on the current device.
...
- **The kind pair**: the intent `kind` on a clear task and a clear question, a two-option choice with no noul labels, reported on its own line, because a kind pair that does not separate says nothing about #156.
...
Two tests prove the first points (9.3).
With a fake Laya that takes 5 s, a Jev Auto `runRouted` with `deps.runId` fixed and a fake clock gives the same record as the same run with the shadow off, compared after removing `ts`, `runId` and every `durationMs` and `ms` (the record carries a fresh id, a timestamp and per-attempt durations, so two runs are never deep-equal as they stand), and it resolves within the fake Jev's latency plus 50 ms.
...
- `shadow.js` keeps an in-memory index by `runId` and `callId` of the rows written since start and the newest 2,000 read at start, so the Decisions tab's poll (5.6) never reads or parses the file; the comparison of 8.4 runs in a `worker_threads` worker over the files and is cached for 60 s or until 100 new rows arrive, so no read of a file at its cap runs on the DSH engine's main thread, which streams the Jev Auto run the person is watching.
...
It shows, from `GET /jev-router/laya/compare?days=7&identity=current` (8.4):
...
Both sample files use `createTrainingStore`, which gains `kind: 'jev' | 'laya'` (default `'jev'`), with the same cap and compaction.
...
- the local classifier is computed and recorded as `local`, and never decides at any rung, because the owner decided that Laya decides and a classifier taught by Jev deciding would put Jev's teaching back into a Laya run;
- an answer gives `authority = answeredBy` and is stored in `provider`, with `teacher: null`;
- no answer or a throw gives the deterministic answer (`code` or `fallback`), never local and never Jev, with the reason `Laya unavailable (<message>); using the deterministic fallback`;
- an answer the teacher closure marks `informative: false` (2.5) gives the deterministic answer too, with the reason `Laya's answer was too flat to use`, in every domain but `outcome_disposition`;
...
- a code-taught domain is decided by its rule exactly as in Jev Auto;
...
- `evidenceFromFeedback` gives nothing for a Laya-decided record, because its dimensions come from Laya's task type too.
...
So Laya's ladder is built now as its **measurement**, a standing per (domain, `laya`), computed by `shadow-stats.js` `standing({ shadowRows, jevSamples, layaSamples, feedback, identity })`.
...
| `config/laya/requirements.in` | `laya[serve]==0.3.20` and `torch==2.14.0` |
...
| `<harness>\engine\laya\installed.json` | `{ laya, torch, torchIndex, cuda, gpu, python, uv, lockSha256, torchWheels, installedAt }` |
...
`config/laya/requirements.lock` is written by `scripts/laya-lock.mjs` with `uv pip compile config/laya/requirements.in --universal --python-version 3.12 --generate-hashes --no-emit-package torch`.
...
4. `Installing PyTorch 2.14.0 for the GPU (CUDA <x>)` or `Installing PyTorch 2.14.0 for the CPU`: `uv pip install --python venv.new torch==2.14.0 --index-url https://download.pytorch.org/whl/<tag>`, or from PyPI for the CPU; progress is the cache folder's size against the wheel size; the installed wheels' hashes go to `installed.json`, so a later change is detectable, because the CUDA wheel cannot be hash-locked from the cloud.
   When the index has no matching wheel for a tag (HTTP 404, or uv's `no matching distribution` / `no solution`), the step tries the next tag of the list, and the card names each tag tried: `PyTorch 2.14.0 has no Windows wheel for cu130; trying cu128.`; only when every listed tag fails does the step fail, offering `Install for the CPU instead`.
...
When torch installs but reports no CUDA on an NVIDIA machine, the card says `Installed for the CPU: PyTorch reports no usable CUDA (driver CUDA <x>). Update the NVIDIA driver, then choose Reinstall.`, and nothing is retried silently.
...
- `fetch_weights.py` calls `laya.router.Router(device='cpu').preload(['english'])`, which downloads exactly the files Laya's `Agent` loads with its own allow-patterns (`laya/agent.py:259-270`) and proves they load, then prints the snapshot commit and each file's size as JSON.
...
```js
export function createLayaSidecar({ harnessDir, dataDir, config /* config.laya */, pins, specs, residency,
                                   ...
                                   isLocalBusy, spawn, fetch, run, log, now, onChange }) -> {
  status(), installed(), isReady(),
  start({ reason, device }), stop({ reason }), restart({ reason, device }), warm(),   // warm: a background start when installed and not failed; never throws
...
Every `TYPESAFE_*` variable and `HF_TOKEN` are removed from the child's environment.
...
| `starting` | waits, same bound | nothing | stops it, unless held |
| `ready` | returns at once | nothing | refused while held: `Laya is deciding for an open Laya Auto run; stop that run first.` |
...
{ "startWithKzh": false, "keepLoaded": false, "idleMinutes": 30, "device": "auto", "shadow": true,
  "measured": { "cpu": { "msPerToken": { "intent": null, "route": null, "review": null }, "ramGB": null, "loadMs": [] },
                "cuda": { "msPerToken": { "intent": null, "route": null, "review": null }, "ramGB": null, "vramGB": null, "loadMs": [] } },
  "lastSelfTest": null }
```
...
export function createResidency() -> {
  set(id, { pid, startedAt, device, busy: () => boolean, held: () => boolean, unload: (why) => Promise<void> }), clear(id), list(),
  othersRamGB(id, { heldOnly }), othersVramGB(id, { heldOnly }),
  yieldFor(id) -> Promise<string[]>,   // unloads every other resident that is neither held nor busy; returns their ids
}
...
- `local.js` registers the llama engine when it is ready, with `held: () => true` because the local model is what the person is using, and clears it on stop; its `unload` is today's trip-and-stop body of `checkMemory` (`local.js:1050`), so the per-model `tripped` bookkeeping and the context sizing after an unload are unchanged when llama itself is the one unloaded, and `local.checkMemory()` stays exported for tests, calling the shared watchdog.
- `laya-sidecar.js` registers Laya the same way, with `held` from 7.6.
...
- Refuse-to-load uses the sum of what will stay: llama's `planFor` and `overBudget` call `residency.yieldFor('llama')` first and then subtract `othersRamGB('llama', { heldOnly: true })` from `maxRamGB`, so llama's context is never sized down for a Laya that nothing holds; Laya refuses to start when its need plus every other resident is over it, and never unloads llama to fit: `Laya needs about 3.3 GB of RAM; the budget leaves 1.2 GB beside qwen3-8b. Stop the local model or raise the RAM budget.`
...
- **Cores**: `LAYA_THREADS` as in 7.4.
  An acting Laya call on the CPU while `local.isBusy()` shares the cores with llama.cpp, and nothing coordinates the two thread pools; the call's live line says so, `Laya is sharing the CPU with <model>, so this call may take longer`, and its deadline already covers it through the measured figures.
- **Budget table**: `ResourceBudget` (`client.js:3597`) adds Laya to Now and Estimated peak, with a breakdown: `2.3 GB (Laya 2.3 GB, llama.cpp not loaded)`.
...
The existing `Jev router` card (`client.js:3187-3191`) adds one line: `Jev calls go to api.typesafe.ai.`, and, when `TYPESAFE_BASE_URL` is set, `TYPESAFE_BASE_URL is set, so Jev calls go to <host>, and the Jev column of the comparisons is that server's answers.`
...
| --- | --- | --- |
| `not_installed` | `Not installed. Needs about <n> GB of free disk while installing and <m> GB after, and the internet once.` then `PyTorch with CUDA will be installed for your <GPU name>.` or `No usable NVIDIA GPU found: Laya will run on the CPU. On a 4-core test machine that took about 20 s to route a task and about 10 s to review each attempt; this PC is not measured yet.` | `Install Laya…` |
| `installing` | `Installing, step <n> of 8: <step name> (<received> of about <total>, or <m> min).` | `Cancel` |
| `install_failed` | `Install failed at step <n> (<step name>): <reason>. The previous install, if any, is untouched.` | `Try again`, `Show log`, and `Install for the CPU instead` when step 4 failed for the GPU |
| `stopped` | `Installed, not running. Laya <version>, English checkpoint <commit 7>, PyTorch <version> (for the GPU, CUDA <x> \| for the CPU).` | `Start`, `Test Laya`, `Remove…` |
| `starting` | `Starting: loading the model on the <GPU \| CPU> (<n> s; the last start took <s> s)…`, with `, measuring Laya on this PC` during the first full warm-up (7.5) | `Stop` |
...
| `ready`, CPU by fallback | `Running on the CPU, not the GPU: <reason>. Measured here: about <a> s to route a task and <b> s to review each attempt.` | `Restart on the GPU`, `Stop`, `Test Laya` |
...
| `restarting` | `Laya stopped unexpectedly (exit code <code>) and is restarting (attempt <n> of 3).` | |
...
| stopped by the budget | `Stopped by the resource budget: <why>.` | `Start` |
| stopped when idle | `Stopped after <n> min without a Laya Auto request. It starts again when Laya Auto needs it; the Jev Auto comparisons never start it.` | `Start` |
| stopped for a local model | `Unloaded so <model> could have the GPU and RAM; it starts again when Laya Auto needs it.` | `Start` |
...
- The counters: `Last 7 days: Laya answered <a> of <b> Jev calls in the background (<c> skipped: <d> not running, <e> starting, <f> queue full, <g> waited too long, <h> gave way to a local model, <i> Jev call failed). Agreement <p>%.`
...
`Downloads once, then works offline: Python 3.12 (about 30 MB, GitHub), PyTorch 2.14.0 (<size>, <source>), Laya 0.3.20 and its libraries (about 100 MB, PyPI), and Laya's English model (0.8 to 1.7 GB, Hugging Face).`
`Needs about <n> GB of free disk while installing and <m> GB after.`
A choice of `GPU: <GPU name> with CUDA <x> (speed not measured on this PC yet; Test Laya measures it after the install)` or `CPU only (smaller download; on a 4-core test machine, about 20 s to route a task and about 10 s to review each attempt)`, and the buttons `Install` and `Cancel`.
...
| `GET /jev-router/laya/compare?days=7\|all&identity=current\|all` | the comparison below, computed in the worker of 5.3 |
...
{ "state": "not_installed | installing | install_failed | stopped | starting | ready | restarting | failed | stopping | disabled",
  ...
  "installed": { "laya": "0.3.20", "torch": "2.14.0+cu128", "cuda": true, "gpu": "NVIDIA GeForce RTX 3050 Laptop GPU",
                 "python": "3.12.11", "weights": { "commit": "<40 hex>", "bytes": 842609220 }, "diskBytes": 0 },
  "expected": { "laya": "0.3.20", "torch": "2.14.0" },
  "running": { "port": 8091, "pid": 1234, "interpreterPid": 1240, "device": "cuda", "deviceWhy": null, "spilling": false, "threads": 6,
               "loadMs": 41000, "ramGB": 1.9, "vramGB": 2.3, "msPerToken": { "intent": 0.2, "route": 0.3, "review": 0.2 },
               "busy": false, "held": ["run:<runId>"], "lastCallMs": 950 },
  "stoppedBecause": null,
  "orphanStopped": null,
  ...
  "selfTest": null, "warnings": [], "logTail": [], "configError": null }
```

`stoppedBecause` is `null | 'idle' | 'budget' | 'yielded'` with the model's name for `yielded`, and `orphanStopped` is `null | { pid, ramGB }` (7.5).
...
{ "identity": "laya-0.3.20|english|<commit 12>|adapter-1|corr:choice:11+=3.27|margin:0.1",
  ...
  "standing": [ "the rows of 6.7, newest per domain" ] }
```

`meanDifference` is filled for nouls and scores, `atBar` for nouls with a bar per provider (`{ n, agree }`), `inTopTwo` for choices; `latency` holds median milliseconds per phase; a domain with no rows of a source has `{ "n": 0 }` there, never a rate.
...
`scripts/red-check.mjs <test files...> [--base <commit>]` proves the first half:

- It adds a temporary worktree at the base (default `8e5d376`) and links the current `plugins/jev-router/node_modules` into it (a directory junction on Windows, a symlink elsewhere), because `node_modules` is untracked and without it every test that imports `jev.js` or `index.js` fails on `@typesafe-ai/sdk` or `@deepseek-ai/schemastery`, never by assertion.
- It copies in the given test files with `test/fixtures`, runs `node --test --test-reporter=tap` on each, and reads the result of every test, not of every file.
...
- It prints the kind of each failure, and accepts three: an assertion, a missing module (`ERR_MODULE_NOT_FOUND`) only for a new module's own tests, and a missing export (`SyntaxError: The requested module ... does not provide an export named ...`) for a new export of an existing module.
  An extended test file imports a new export of an existing module through a namespace import (`import * as training from '../training.js'`) or a dynamic `import()` inside the test, so the other tests of the file still load and fail by assertion; a static named import of a new export would fail the whole file at link time.
...
export async function startFakeLaya({
  ...
  device = 'cpu',
}) -> { url, port, requests /* every parsed body, with arrival and finish times */, busy(), close(),
...
- `test/jev.test.js` (extended, G1): the new form sends `defaultModel` and `retry.maxRetries` from the record and `timeout` per phase; `phase` rides the per-call options; a failed call calls `onError` with its class and never `onTrace`; `onCall` fires once per call with the unadapted state and deep-frozen questions, `assess`'s third argument reaches it as `context`, its settle function gets the trace, and a never-settling or throwing hook never delays or breaks the return; the criteria constants are frozen; `route()`, `intent()` and `assess()` return `uninformative` from the answers' flags, and `[]` for Jev; `profileFromAnswers` reads `supportingSkill` and `verificationChecks` (`'always'` always adds checks), leaves out flat scores and choices and keeps flat nouls; `assess` returns the model; the old form still works.
...
- `test/local.test.js` (extended, G3): `isBusy()` while a stream is open; llama registers in the residency; `planFor` calls `yieldFor` and subtracts held residents' RAM only; with Laya loaded but not held, a local model start gets the same context and fit target as with no Laya, and a busy llama is never the watchdog's victim because of Laya's share.
...
- `test/shadow.test.js` (new, G4): with a 5 s fake Laya, `deps.runId` fixed and a fake clock, a Jev Auto `runRouted` record equals the shadow-off run's once `ts`, `runId`, `durationMs` and `ms` are removed, and resolves within the fake Jev's latency plus 50 ms, for a Laya that answers, throws, hangs, returns garbage, and an adapter that throws; two consecutive Jev Auto runs send Jev request bodies byte-identical with the shadow on and off; rows pair by `callId` in either arrival order; each skip reason is recorded and counted; a failed Jev call withdraws its job, and a rotated 429 yields one compared row; review rows carry the `context` numbers; tool-parameter answers are recorded as indexes; a marker string placed in the task, an answer, a tool description and a tool option key never appears in the file; intent rows carry `runId: null`; learning off writes nothing; compaction keeps the newest 10,000 rows; on a file at the cap, `read({ runId })` takes under 20 ms of main-thread time.
...
2. **Plant the checkpoint**: `test/e2e/make_ckpt.py` builds the English architecture with `laya.common.build_model` (ModernBERT-large: hidden 1024, 28 layers, intermediate 2624, 16 heads, vocab 50368; 421.29M parameters; fp16 safetensors of 842,609,220 bytes; the English `rl_agent_config.json` with its real temperatures, `choice:11+` 0.1006 included) and `test/e2e/plant_hf_cache.py` lays it out as `models/laya/hf/hub/models--convaiinnovations--laya/snapshots/<40 hex>/` with `refs/main`, the tokenizer being OLMo's GPT-NeoX BPE as a proxy (fetched from GitHub raw, pinned by SHA-256); then `fetch_weights.py` runs on it in offline mode, which loads it once through Laya and writes `weights.json` afterwards (7.3), exactly as the owner's download does.
3. **Start through the real supervisor** on the CPU: readiness, `device: 'cpu'`, the temperature warning parsed, a wrong key refused with 401 and no retry, and a connection to the machine's non-loopback address refused.
4. **KzH's real bodies** through `createJev({ provider: providers.laya, client: layaClient.client('act') })`: intent, task group, resource and judgments, review; no 4xx (labels accepted), every question answered, the answers parse into a profile and an assessment, the model relabelled, no request at the context limit; the timings go to `e2e-report.json`, expected on 4 vCPU at about 1.5 s, 16 to 18 s, 1.5 s and 9 s.
5. **The gate against the real lock**: a route call with a 3 s deadline rejects at 3 s, the slot stays held until the server's own response arrives (about 16 to 20 s later), and then an acting call runs alone; a route call whose caller aborts at 2 s does the same, and an intent call queued behind it and aborted before it is sent never reaches the server.
6. **Shadow**: a Jev Auto `runRouted` with a fake Jev (300 ms) and fake agents finishes in about Jev's time plus the agents' while the real Laya computes, and the shadow row lands later.
7. **Laya Auto**: a `runRouted` on a temporary git workspace with a stub agent completes with no TypeSafe contact (fetch and constructor spies); with random weights nearly every answer is flat, so the rules fill them and the heading and lines say so.
8. **Supervision**: the first start's full warm-up records a figure for every phase and a RAM peak; the working set is positive and under 3.5 GB; killing the interpreter mid-call fails the call with the reason, the state goes `restarting` then `ready`, and the next call answers; a tiny RAM budget trips the watchdog on an unheld Laya only; a 1-minute idle setting stops it while shadow requests keep arriving, and `ensureReady` starts it again; a Laya Auto run held open for 2 minutes keeps it loaded.
9. **Render check**: `test/e2e/check_render.py`, with Laya's own `build_sequence` and the proxy tokenizer, finds no option of any KzH choice cut and no view cut.

It cannot show answer quality, GPU behaviour, or Windows process handling; the desktop test does.
...
12. **Feedback and integrity.**
    Use `good pick` and `misread my question` on a few runs of each mode, then run `node scripts\laya-integrity-check.mjs`: no Laya-decided run id in `routing-samples.jsonl`, no capability evidence but reliability from Laya runs, and "Saved by Jev" unchanged by Laya rows.
13. **Bring back** one file: run `node scripts\laya-export.mjs`, which writes `laya-export-<date>.json` and prints what it holds: the shadow rows, both sample stores, the standings, `laya.json`, `installed.json`, `weights.json`, the last 500 lines of `laya-serve.log`, and the `history.jsonl` rows reduced to `runId`, the routing labels and numbers, each attempt's agent, role and stop reason, the final status and the assessment actions, every string passed through `export.js` `redactSecrets`.
...
| G2 laya-wire | `laya-questions.js`, `laya-selfcheck.js` (new), `test/fixtures/fake-laya-serve.mjs`, `test/fixtures/kzh-bodies.json` (new), `test/laya-questions.test.js`, `test/laya-selfcheck.test.js` (new) | 4.2, 4.3, 4.6 with `createProbe` and the warm-up bodies, 9.2 | 9.3 G2 items | none |
