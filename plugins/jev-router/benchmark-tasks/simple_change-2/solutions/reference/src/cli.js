...
function main(argv) {
  ...
  if (options.deprecated) console.error('warning: --verbose is deprecated, use --debug')
  if (options.help) {
    console.log(USAGE)
    return
  }
  if (options.file === null) throw new Error('a file is required')
  console.log(countLines(options.file, { verbose: options.debug, blank: options.blank }))
}
